...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to weights that should be loaded before training.",
    )

    parser.add_argument(
        "--finetune",
        action="store_true",
        help="Unfreeze the top backbone layers and fine-tune the model.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Number of backbone layers to unfreeze during fine-tuning.",
    )
    parser.add_argument(
        "--augmentation_mode",
        type=str,
        ...
        help="Training augmentation configuration.",
    )
...
def run_normal_training(
    ...
    # Fine-tuning must start from previously trained weights.
    if args.finetune and not args.resume_weights:
        raise ValueError(
            "--finetune requires --resume_weights. "
            "Provide the path to the best weights from an earlier run."
        )

    # Load the previous checkpoint before unfreezing layers.
    if args.resume_weights:
        resume_path = Path(args.resume_weights)

        if not resume_path.exists():
            raise FileNotFoundError(
                f"Resume weights file does not exist: {resume_path}"
            )
    ...
    # Unfreeze the final backbone layers for fine-tuning.
    if args.finetune:
        print(
            f"\nUnfreezing the top "
            f"{args.unfreeze_top_n} backbone layers..."
        )

        model = unfreeze_top_n(
            model,
            n=args.unfreeze_top_n,
        )

        # Recompile because trainable layers changed.
        model.compile(
        ...
        print(
            f"Model recompiled for fine-tuning with "
