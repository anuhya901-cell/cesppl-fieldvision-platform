...
def dashboard_summary(
    ...
    with _get_connection(db_path) as connection:
        total_uploads_row = connection.execute(
            """
        ...
        active_classes_row = connection.execute(
            """
            SELECT COUNT(
                DISTINCT class_name
            ) AS active_classes
        ...
        uploads_today_row = connection.execute(
            """
    ...
    return {
        "total_uploads": int(
            total_uploads_row["total_uploads"]
        ),
        "active_classes": int(
            active_classes_row["active_classes"]
        ),
        "uploads_today": int(
            uploads_today_row["uploads_today"]
        ),
    }