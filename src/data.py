...
def _read_split_csv(split_name):
    ...
    if not required_columns.issubset(
        ...
        raise ValueError(
            f"{csv_path.name} must contain "
            f"filename and class columns."
        )
...
def _load_image_with_path(
    image_path,
    label,
    img_size,
):
    image, label = _load_image(
...
def get_class_weights():
    ...
    unknown_classes = sorted(
        set(train_dataframe["class"])
        - set(class_names)
    )

    if unknown_classes:
        raise ValueError(
            "Unknown classes found in "
            f"train.csv: {unknown_classes}"
        )

    labels = (
        train_dataframe["class"]
        .map(class_to_index)
        .astype(np.int32)
        .to_numpy()
    )
